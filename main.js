...
function loadJson() {
  let response = fetch(fichierJson)
    ...
    .then(function (data) {
      ...
      mediaSet = data.media;
      loadTags(photographersSet);
      affTags(tagSet);
      displayPhotographersMain(photographersSet);
      // DOM Elements
      ...
      pcards.forEach((a) => a.addEventListener("click", loadMod1));
    })
};

// chargement des tags à partir des photographes
function loadTags(photographersSet) {
  let appendTag = true
...
function displayPhotographersMain(photographersSet) {
  effaceCards();
  photographersSet.forEach(p => {
    if (tagSet[0].state) {
...
function displayPhotographer(p) {
  ...
  let cardPhotographe = document.createElement('button');
  //  cardPhotographe.setAttribute('href', "#mod1");
...
function loadMod1(event) {
  ...
  photographersSet.forEach(i => {
    if (photographersSet[i].id == p) {
      let pSel = Photographer(photographersSet[i]);
    }
  });
  ...
  w_img.setAttribute('src', "../public/img/PhotographersIDPhotos/" + pSel.portrait);
  w_name.setAttribute('id', pSel.id);
  w_name.textContent = pSel.name;
  w_place.textContent = pSel.city + ", " + pSel.country;
  w_alt.textContent = pSel.alt;
  //  for (let i = 0; i < p.tags.length; i++) {
  pSel.tags.forEach(t => {
    let tagPhotographe = document.createElement('button');
    let wtagLib = libTag(pSel.tags[t]);
    for (let i = 1; i < tagSet.length; i++) {
      tagPhotographe.setAttribute('id', pSel.tag[t]);
      if (tagSet[i] == pSel.tags[t]) {
